...
function processRawData(rawData: any[]): Project[] {
          ...
          Object.keys(groupedData).forEach(id => {
            ...
            rows.forEach((r: any) => {
              ...
              const usable = parseFloat(getValue(r, ['Usable Area (sq.m.)', 'Usable Area', 'Size'])) || 0;
              const land = parseFloat(getValue(r, ['Land Area (sq.w.)', 'Land Area'])) || 0;
              const total = parseFloat(getValue(r, ['Total units', 'Total Units', 'Units'])) || 0;
              const sold = parseFloat(getValue(r, ['Sold Units', 'Sold'])) || 0;
              const price = parseFloat(getValue(r, ['Avg. Price (Units)', 'Price', 'Avg Price', 'Avg. Price'])) || 0;
              let speed6m = parseFloat(getValue(r, ['Sale Speed (6 เดือน)', 'Sale Speed 6m', 'Speed 6m', 'Sale Speed (6 Months)'])) || 0;
              const speed = parseFloat(getValue(r, ['Sale Speed', 'Speed', 'Total Sale Speed'])) || 0;
              ...
              Object.keys(r).forEach(key => {
                ...
                if (/^H[12]\.\d+/.test(cleanKey)) {
                  ...
                  if (!isNaN(val)) history[cleanKey] = val;
                }
              });
              ...
              if (speed6m === 0 && Object.keys(history).length > 0) {
                const hKeys = Object.keys(history).sort((a, b) => {
                  const parseKey = (k: string) => {
                    ...
                    return { half: parseInt(m[1]), year: parseInt(m[2]), is12m: k.toLowerCase().includes('(12m)') };
                  };
                  const aV = parseKey(a); const bV = parseKey(b);
                  if (aV.is12m !== bV.is12m) return bV.is12m ? 1 : -1;
                  if (aV.year !== bV.year) return bV.year - aV.year;
                  return bV.half - aV.half;
                });
                if (hKeys.length > 0) speed6m = history[hKeys[0]];
              }
              ...
              let priceDisplay = '-';
              if (price > 0) priceDisplay = (price < 1000 ? price : price / 1000000).toFixed(2) + ' MB';

              subUnits.push({
                type, usableArea: usable > 0 ? usable.toFixed(1) : '-', landArea: land > 0 ? land.toFixed(1) : '-',
                totalUnits: total, soldUnits: sold, percentSold: perSold, price, priceStr: priceDisplay,
                launchDate, saleSpeed: speed.toFixed(2), saleSpeed6m: speed6m.toFixed(2), history
              });
            });
            ...
            processedProjects.push({
              projectId: id, lat, lng, code: code || 'XX', name: name || 'Unknown', developer,
              subUnits, totalUnits: projTotalUnits, soldUnits: projSoldUnits,
              percentSold: projPercentSold.toFixed(1), priceRange,
              saleSpeed6m: projSaleSpeed6m.toFixed(2), saleSpeed: projSaleSpeed.toFixed(2)
            });
          });
